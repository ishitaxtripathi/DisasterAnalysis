...
# AS There were 19 values higher than e9
...
Combining into America
```{r}
america <- rbind(guatemala_c,drepublic_c,mexico_c)
```

#Preprocessing Visualization America
```{r}
...
Dominican Republic  model 2 - 0.54 death ~ - c(Event, Deaths) model 1 perform 0.88
```{r}
...
americas <- rbind(guatemala_c, drepublic_c, mexico_c)
americas$cluster_id <- as.integer(as.character(americas$cluster_id))
data_for_clustering <-americas[, c("cluster_id", "Deaths", "Monetary.LossUSD")]
