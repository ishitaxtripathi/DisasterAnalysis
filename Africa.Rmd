...
Directory
```{r}
setwd("D:/Post graduation/Semester 1/Data Analysis/Coursework/Group 18/Africa")
```
Library
```{r}
library(dplyr)
...
```
Split parameter and model (Continent)
```{r}
...
```{r}
set.seed(123)
split_indices <- createDataPartition(tunisia_c$Year, p = 0.7, list = FALSE)
training_data_tunisia <- tunisia_c[split_indices, ]
testing_data_tunisia <- tunisia_c[-split_indices, ]
```

```{r}
# Imbalanced Distribution: There's considerable variation in the number of observations across different years. Some years have only a few observations while others have many more. View distubution by table(senegal_c$Year)
...
Tanzania - model2- 0.68 death ~ - c(Event,Deaths)
```{r}
...
Zambia - model2- 0.89 death ~ - c(Event,Deaths)
```{r}
...
Senegal - model2- 0.05 death ~ - c(Event,Deaths)
```{r}
...
Tunisisa - model2- 0.38 death ~ - c(Event,Deaths,DataCards)
```{r}
y <- training_data_tunisia$Deaths
X <- subset(training_data_tunisia, select = - c(Event,Deaths,DataCards ))  
