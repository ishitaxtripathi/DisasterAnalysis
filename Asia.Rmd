...
#Preprocessing Visualization (Asia)
```{r}
asia_perc <- asia %>%
  mutate(
...
enet_model_srilanka <- cv.glmnet(X_train_std, y_train, alpha = 0.5) 

best_lambda <- enet_model_srilanka$lambda.min

y_pred <- predict(enet_model_srilanka, newx = X_test_std, s = best_lambda)
...
asias <- rbind(turkey_c, vietnam_c, yemen_c, pakistan_c, srilanka_c)
asias$cluster_id <- as.integer(as.character(asias$cluster_id))
data_for_clustering <-asias[, c("cluster_id", "Deaths", "Monetary.LossUSD")]
